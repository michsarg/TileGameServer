...
live_idnums = [] # list of idnums
clientdict = {} # key: idnum, value: names
connectiondict = {} # key: idnum, value: connection
...
def client_handler(connection, address, idnum):
  ...
  if (len(live_idnums) == 2):
    for x in connectiondict:
      for _ in range(tiles.HAND_SIZE):
        tileid = tiles.get_random_tileid()
        connectiondict.get(x).send(tiles.MessageAddTileToHand(tileid).pack())
      connectiondict.get(x).send(tiles.MessageGameStart().pack())
...
def start():
  ...
  while True:
    ...
    # inform all existing players that this player has joined (Not working?)
    for x in connectiondict.values():
      if x != connection:
        x.send(tiles.MessagePlayerJoined(name, idnum).pack())

    # inform present player of past (WORKING)
